...
def create_summarizer(model_name):
    if model_name not in model_cache:
        try:
            summarizer = pipeline('summarization', model=model_name, max_length=50, num_beams=4, early_stopping=True)
...
def generate_summaries(paragraph_texts, summarizers):
    ...
    for model_name, summarizer in summarizers.items():
        if summarizer is not None:
            try:
                summary = summarizer(paragraph_texts, max_length=300, num_beams=4, early_stopping=True)
